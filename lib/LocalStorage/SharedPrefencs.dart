...
class TokenGetter {
  ...
  readUserInfo() async {
    ...
    UserInfo user = UserInfo.fromJson(
        json.decode(prefs.getString(AppConstants.USER_INFO)!));
    return user;
  }
  ...
  readDialCode() async {
    ...
    List<String> DialCode =
        (jsonDecode(prefs.getString(AppConstants.USER_DIAL_CODE)!)
                as List<dynamic>)
            .cast<String>();
    return DialCode;
  }
  ...
  readActivites() async {
    ...
    Activities activities = Activities.fromJson(
        json.decode(prefs.getString(AppConstants.ACTIVITIES)!));
    return activities;
  }
  ...
  readRelationList() async {
    final prefs = await SharedPreferences.getInstance();
    RelationList _RelationList = RelationList.fromJson(
        json.decode(prefs.getString(AppConstants.RELATION)!));
