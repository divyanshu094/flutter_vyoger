...
class _TravelCalenderState extends State<TravelCalender>
  ...
  setCalendarValue(CalenderEventResponseModel events) async {
    // print('extracted data 21 ${events.data.reversed.toList()}');

    var extracted_events = await getTask1(events.data.reversed.toList());
    // print('extracted data $extracted_events');
    this.setState(() {
  ...
  Future<Map<DateTime, List>> getTask1(List<CalendarEvent> event) async {
    ...
    for (int i = 0; i < event.length; i++) {
      ...
      // print('event[i].fromDate 1 ${createTime}');
      var endTime = DateTime.parse(event[i].toDate);
      // print('event[i].fromDate 2 ${endTime}');
      var difference = endTime.difference(createTime).inDays;
      // print('event[i].difference 2 ${difference}');
      var original = mapFetch[createTime];
      // print('event[i].original 2 ${original}');
      if (original == null) {
      ...
      // print('event[i].extractedDays 2 ${extractedDays}');
      for (int j = 0; j < extractedDays.length; j++) {
        var mydate = mapFetch[extractedDays[j]];
        // print('event[i].mydate 2 ${mydate}');
        if (mydate == null) {
    ...
    // print('event[i].mapFetch 2 ${mapFetch}');
    return mapFetch;
  }
  ...
  Widget _buildTableCalendarWithBuilders() {
    ...
    //     markersPositionLeft: null,
    //     markersPositionRight: null,
    //   ),
    ...
    //     titleTextBuilder: (DateTime date, locale) {},
    //   ),
