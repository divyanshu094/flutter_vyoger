...
class UsernameBloc {
  late bool isEmailValid, isPasswordValid;
  late String email, password;
  late String confirmMail;
  ...
  Stream<UserToken> get getIsValidPass => _isPassValidController.stream;
  ...
  Future passwordValidate(BuildContext context) async {
    if (password.isEmpty) {
      showDefaultSnackbar(context, 'Password cannot be Empty');
    } else {
  ...
  UserBiometricAuth(email, password) async {
    Credential UserCredentials =
        Credential(username: email, password: password, checkbiometric: false);
  ...
  sendResetEmail() async {
    if (confirmMail.isEmpty) {
      ...
      ForgetPassModel passModel =
          await _repository.send_resetEmail(confirmMail);
