...
class _DashboardState extends State<Dashboard> {
  ...
  late UserInfo info; // = UserInfo();
  String UserName = '';
  String? ProfileImage;

  ApiProvider _appApiProvider = ApiProvider();
  late List<DragDropModel> models;
  late SharedPreferences prefs;
  bool showloader = false;
  final box = GetStorage();
  late Credential userCred;
  ...
  firstTimeUser() async {
    try {
      bool value = await box.read(AppConstants.FIRST_TIME);
      if (value) {
        return true;
      } else {
        return false;
  ...
  getUserAuthBiometric() async {
    bool first_time = await firstTimeUser();
    if (first_time) {
      box.write(AppConstants.FIRST_TIME, false);
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            GestureDetector(
              onTap: () {
                showCustomDialogCityClass(context, SOS(), 350);
              },
              child: Container(
                height: 30,
                width: 30,
