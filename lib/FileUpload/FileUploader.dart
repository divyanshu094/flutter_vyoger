...
class FileUpload {
  ...
  uploadFile(image, _uploadUrl, _tasks,
      {required Function(String, String) onstart}) async {
    ...
    if (image != null) {
      ...
      var taskId = await FlutterUploader().enqueue(MultipartFormDataUpload(
        url: url,
...
class UploadItem {
  ...
  UploadItem copyWith(
          {required UploadTaskStatus status, required int progress}) =>
      UploadItem(
          id: this.id,
          tag: this.tag,
          type: this.type,
